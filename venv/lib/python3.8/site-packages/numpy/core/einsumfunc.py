    ...
    *operands : list of array_like
    ...
    for tnum, term in enumerate(input_list):
        ...
        for cnum, char in enumerate(term):
            ...
            if (
                char in dimension_dict
                and dimension_dict[char] == 1
                or char not in dimension_dict.keys()
            ):
    ...
    for cnum, contract_inds in enumerate(path):
        ...
        do_blas = (
            False
            if len(idx_removed & bcast)
            else _can_dot(tmp_inputs, out_inds, idx_removed)
        )
    ...
    overall_contraction = f"{input_subscripts}->{output_subscript}"
...
def einsum(*operands, out=None, optimize=False, **kwargs):
    ...
    * Chained array operations, in efficient calculation order, :py:func:`numpy.einsum_path`.
    ...
    for num, contraction in enumerate(contraction_list):
        ...
        if blas:
            ...
            if (tensor_result != results_index) or handle_out:
                if handle_out:
                    kwargs["out"] = out
                new_view = c_einsum(f'{tensor_result}->{results_index}', new_view, **kwargs)
    ...
    return out if specified_out else asanyarray(operands[0], order=output_order)
