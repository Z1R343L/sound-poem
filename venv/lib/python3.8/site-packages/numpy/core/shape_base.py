...
def _block_check_depths_match(arrays, parent_index=[]):
    ...
    if type(arrays) is tuple:
        ...
        raise TypeError(
            f'{_block_format_index(parent_index)} is a tuple. Only lists can be used to arrange blocks, and np.block does not allow implicit conversion from tuple to ndarray.'
        )
        ...
        for index, ndim, size in idxs_ndims:
            ...
            if len(index) != len(first_index):
                raise ValueError(
                    f"List depths are mismatched. First element was at depth {len(first_index)}, but there is an element at depth {len(index)} ({_block_format_index(index)})"
                )
