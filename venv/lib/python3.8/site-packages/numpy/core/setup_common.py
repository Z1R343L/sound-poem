...
def set_sig(sig):
    ...
    FUNC_CALL_ARGS[funcname] = ", ".join(f"({arg}) 0" for arg in args)
...
C99_FUNCS_SINGLE = [f'{f}f' for f in C99_FUNCS]
C99_FUNCS_EXTENDED = [f'{f}l' for f in C99_FUNCS]
C99_COMPLEX_TYPES = [
...
def check_long_double_representation(cmd):
    ...
    if sys.platform == "win32" and not mingw32():
        ...
        cmd.compiler.set_executables(
            ...
            linker_so=f'{newcompiler} -shared',
        )
...
def long_double_representation(lines):
    ...
    if saw is not None:
        raise ValueError(f"Unrecognized format ({saw})")
    else:
        # We never detected the after_sequence
        raise ValueError(f"Could not lock sequences ({saw})")
