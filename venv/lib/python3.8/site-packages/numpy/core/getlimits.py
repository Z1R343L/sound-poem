...
class MachArLike:
    ...
    def __init__(self, ftype, *, eps, epsneg, huge, tiny,
        ...
        self._smallest_subnormal = smallest_subnormal or nextafter(
...
def _get_machar(ftype):
    ...
    ma_like = _KNOWN_TYPES.get(key[:10]) if ftype == ntypes.longdouble else None
    if ma_like is None:
    ...
    warnings.warn(
        f'Signature {key} for {ftype} does not match any known type: falling back to type probe function',
        UserWarning,
        stacklevel=2,
    )
...
class iinfo:
    ...
    def __repr__(self):
        return f"{self.__class__.__name__}(min={self.min}, max={self.max}, dtype={self.dtype})"
