...
def _extendLine(s, line, word, line_width, next_line_prefix, legacy):
    ...
    if legacy > 113 and len(line) <= len(next_line_prefix):
        needs_wrap = False
...
def _formatArray(a, format_function, line_width, next_line_prefix,
    ...
    def recurser(index, hanging_indent, curr_width):
        ...
        next_width = curr_width if legacy <= 113 else curr_width - len(']')
        a_len = a.shape[axis]
        ...
        if axes_left == 1:
            ...
            if show_summary:
                ...
                line += ", " if legacy <= 113 else separator
...
class FloatingFormat:
    ...
    def __init__(self, data, precision, floatmode, suppress_small, sign=False,
        ...
        self._legacy = legacy
        if self._legacy <= 113 and data.shape != () and sign == '-':
            sign = ' '
    ...
    def fillFormat(self, data):
        ...
        if len(finite_vals) == 0:
            ...
            self.pad_left = 3 if self._legacy <= 113 else max(len(s) for s in int_part)
...
class DatetimeFormat(_TimelikeFormat):
    ...
    def __call__(self, x):
        return self._format_non_nat(x) if self.legacy <= 113 else super().__call__(x)
