...
def test_diophantine_fuzz():
    ...
    for ndim in range(10):
        ...
        while min(feasible_count, infeasible_count) < min_count:
            ...
            A = tuple(int(rng.randint(1, A_max+1, dtype=np.intp)) for _ in range(ndim))
            U = tuple(int(rng.randint(0, U_max+2, dtype=np.intp)) for _ in range(ndim))
...
def test_internal_overlap_fuzz():
    ...
    while min(overlap, no_overlap) < min_count:
        ...
        strides = tuple(rng.randint(-1000, 1000, dtype=np.intp) for _ in range(ndim))
        shape = tuple(rng.randint(1, 30, dtype=np.intp) for _ in range(ndim))
...
class TestUFunc:
    ...
    def test_binary_ufunc_accumulate_fuzz(self):
        def get_out_axis_size(a, b, axis):
            if axis is None:
                return (a.size, False) if a.ndim == 1 else ('skip', False)
            else:
