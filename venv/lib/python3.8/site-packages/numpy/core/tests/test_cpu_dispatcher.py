...
def test_dispatcher():
    ...
    for feature in reversed(targets):
        ...
        if not highest_sfx:
            highest_sfx = f"_{feature}"
        all_sfx.append("func" + "_" + feature)

    test = _umath_tests.test_dispatch()
    assert_equal(test["func"], f"func{highest_sfx}")
    assert_equal(test["var"], f"var{highest_sfx}")

    if highest_sfx:
        assert_equal(test["func_xb"], f"func{highest_sfx}")
        assert_equal(test["var_xb"], f"var{highest_sfx}")
    else:
