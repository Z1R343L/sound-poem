...
class TestTypes:
    ...
    def test_promote_types_strings(self, swap, string_dtype):
        ...
        assert_equal(promote_types('bool', S), np.dtype(f'{S}5'))
        assert_equal(promote_types('b', S), np.dtype(f'{S}4'))
        assert_equal(promote_types('u1', S), np.dtype(f'{S}3'))
        assert_equal(promote_types('u2', S), np.dtype(f'{S}5'))
        assert_equal(promote_types('u4', S), np.dtype(f'{S}10'))
        assert_equal(promote_types('u8', S), np.dtype(f'{S}20'))
        assert_equal(promote_types('i1', S), np.dtype(f'{S}4'))
        assert_equal(promote_types('i2', S), np.dtype(f'{S}6'))
        assert_equal(promote_types('i4', S), np.dtype(f'{S}11'))
        assert_equal(promote_types('i8', S), np.dtype(f'{S}21'))
        # Promote numeric with sized string:
        assert_equal(promote_types('bool', f'{S}1'), np.dtype(f'{S}5'))
        assert_equal(promote_types('bool', f'{S}30'), np.dtype(f'{S}30'))
        assert_equal(promote_types('b', f'{S}1'), np.dtype(f'{S}4'))
        assert_equal(promote_types('b', f'{S}30'), np.dtype(f'{S}30'))
        assert_equal(promote_types('u1', f'{S}1'), np.dtype(f'{S}3'))
        assert_equal(promote_types('u1', f'{S}30'), np.dtype(f'{S}30'))
        assert_equal(promote_types('u2', f'{S}1'), np.dtype(f'{S}5'))
        assert_equal(promote_types('u2', f'{S}30'), np.dtype(f'{S}30'))
        assert_equal(promote_types('u4', f'{S}1'), np.dtype(f'{S}10'))
        assert_equal(promote_types('u4', f'{S}30'), np.dtype(f'{S}30'))
        assert_equal(promote_types('u8', f'{S}1'), np.dtype(f'{S}20'))
        assert_equal(promote_types('u8', f'{S}30'), np.dtype(S+'30'))
        # Promote with object:
...
class TestFromiter:
    ...
    def test_failed_itemsetting(self):
        ...
        iterable = ((2, 3, 4) for _ in range(5))
...
class TestClip:
    ...
    def fastclip(self, a, m, M, out=None, casting=None):
        if out is None:
            return a.clip(m, M) if casting is None else a.clip(m, M, casting=casting)
        if casting is None:
