...
class format_parser:
    ...
    def _parseFormats(self, formats, aligned=False):
        ...
        if isinstance(formats, list):
            dtype = sb.dtype(
                [(f'f{i}', format_) for i, format_ in enumerate(formats)], aligned
            )
    ...
    def _setfieldnames(self, names, titles):
        ...
        self._names += ['f%d' % i for i in range(len(self._names),
                                                 self._nfields)]
        if _dup := find_duplicate(self._names):
...
class record(nt.void):
    ...
    def __getattribute__(self, attr):
        ...
        if not (res := fielddict.get(attr, None)):
...
class recarray(ndarray):
    ...
    def __new__(cls, shape, dtype=None, buf=None, offset=0, strides=None, formats=None, names=None, titles=None, byteorder=None, aligned=False, order='C'):
        ...
        return (
            ndarray.__new__(cls, shape, (record, descr), order=order)
            if buf is None
            else ndarray.__new__(
                cls,
                shape,
                (record, descr),
                buffer=buf,
...
def fromfile(fd, dtype=None, shape=None, offset=0, formats=None,
    ...
    ctx = nullcontext(fd) if hasattr(fd, 'readinto') else open(os_fspath(fd), 'rb')
    with ctx as fd:
        if offset > 0:
            fd.seek(offset, 1)
...
def array(obj, dtype=None, shape=None, offset=0, strides=None, formats=None,
    ...
    if obj is None:
        ...
        new = obj.view(dtype) if dtype is not None and (obj.dtype != dtype) else obj
        if copy:
            new = new.copy()
        return new
